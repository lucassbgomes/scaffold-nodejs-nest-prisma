...
describe('Edit Seo Override Controller (e2e)', () => {
  ...
  it('should be able to edit a seo override', async () => {
    ...
    const response = await request(app.getHttpServer())
      .patch(`/seo-overrides/${seoOverride.id}`)
    ...
    const seoOverrideEdited = await prisma.seoOverride.findUnique({
      where: { id: seoOverride.id.toString() },
    });

    expect(response.statusCode).toEqual(204);
    expect(seoOverrideEdited?.title).toEqual('Title Edited');
  });
});
